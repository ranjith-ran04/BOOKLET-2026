...
//            }
          ...
          <body>
            <div class="container">
              ...
              <table class="main-info">
                ...
                <tr><td>Taluk</td><td>${
                  !summary.taluk ? "-" : summary.taluk
                }</td></tr>
                <tr><td>District</td><td>${
                  !summary.district ? "-" : summary.district
                }</td></tr>
                <tr><td>Pincode</td><td>${
                  !summary.pincode ? "-" : summary.pincode
                }</td></tr>
                <tr><td>Phone/Fax</td><td>${
                ...
                <tr><td>Website</td><td>${
                  !summary.website ? "-" : summary.website
                }</td></tr>
                <tr><td>Anti-Ragging Phone No</td><td>${
                  !summary.antiphone ? "-" : summary.antiphone
                }</td></tr>
              </table>
              <table class="main-info2">
                <tr><td>Bank A/c No</td><td>${
                  !summary.accno ? "-" : summary.accno
                }</td></tr>
                <tr><td>Bank Name</td><td>${
                  !summary.bankname ? "-" : summary.bankname
                }</td></tr>
                <tr><td>Distance in KMS from Dist. HQ</td><td>${
                  !summary.distance ? "-" : summary.distance
                }</td></tr>
                <tr><td>Nearest Railway Station</td><td>${
                  !summary.rly ? "-" : summary.rly
                }</td></tr>
                <tr><td>Distance in KMS from Nearest Railway Station</td><td>${
                  !summary.rlydistance ? "-" : summary.rlydistance
                }</td></tr>
                <tr><td>Minority Status</td><td>${
                  !summary.minority ? "-" : summary.minority
                }</td></tr>
                <tr><td>Autonomous Status</td><td>${
                  !summary.autonomo ? "-" : summary.autonomo
                }</td></tr>
              </table><br>
              <table class="Hostel">
                <tr><th>Hostel Facilities</th><th>Boys</th><th>Girls</th></tr>
                <tr><td>Accommodation Available for UG</td><td>${
                  !summary.accb ? "-" : summary.accb
                }</td><td>${!summary.accg ? "-" : summary.accg}</td></tr>
                <tr><td>Permanent or Rental (P/R)</td><td>${
                  !summary.htypeb ? "-" : summary.htypeb
                }</td><td>${!summary.htypeg ? "-" : summary.htypeg}</td></tr>
                <tr><td>Type of Mess (Veg/NV)</td><td>${
                  !summary.messb ? "0" : summary.messb
                }</td><td>${!summary.messg ? "-" : summary.messg}</td></tr>
                <tr><td>Mess Bill</td><td>${
                  !summary.billb ? "0" : summary.billb
                }</td><td>${!summary.billg ? "0" : summary.billg}</td></tr>
                <tr><td>Room Rent</td><td>${
                  !summary.rentb ? "0" : summary.rentb
                }</td><td>${!summary.rentg ? "0" : summary.rentg}</td></tr>
                <tr><td>Electricity Charges</td><td>${
                  !summary.elecb ? "0" : summary.elecb
                }</td><td>${!summary.elecg ? "0" : summary.elecg}</td></tr>
                <tr><td>Caution Deposit</td><td colspan="2">${
                  !summary.caution ? "0" : summary.caution
                }</td>
                <tr><td>Establishment Charges</td><td colspan="2">${
                  !summary.estab ? "0" : summary.estab
                }</td>
                <tr><td>Admission Fees</td><td colspan="2">${
                  !summary.adm ? "0" : summary.adm
                }</td>
                <tr><td>Transport Facilities (Y/N)</td><td colspan="2">${
                  !summary.trans ? "-" : summary.trans
                }</td>
                <tr><td>Min Transport Charges</td><td colspan="2">${
                  !summary.mintrans ? "0" : summary.mintrans
                }</td>
                <tr><td>Max Transport Charges</td><td colspan="2">${
                  !summary.maxtrans ? "0" : summary.maxtrans
                }</td>
              </table>
              <table class="course">
              ...
              ${
                proposed.length > 0 || courses.length
                  ? `
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=${c_code}_booklet.pdf`
    );
